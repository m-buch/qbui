  ...
  <div>
    ...
    <div class="p-0 md:p-6">
      ...
      <div v-else-if="store.filtered.length === 0" class="py-8 text-center text-gray-400">
        ...
        <p class="text-sm mt-4">{{ filterLabel }} items will appear here.</p>
      </div>
      <div v-else>
        <ContextMenu v-for="torrent in store.filtered" :key="torrent.hash">
          ...
          >
            ...
            <ContextMenuSub>
              <ContextMenuSubTrigger class="hover:bg-blue-500/80">
                Set Category
              </ContextMenuSubTrigger>
              <ContextMenuSubContent
                class="w-48 dark:bg-neutral-800/80 dark:backdrop-blur-sm dark:border-neutral-700"
              >
                <Input
                  v-model="newCategory"
                  placeholder="Create a new category..."
                  class="h-8 px-2 text-sm dark:bg-neutral-900/60 border border-neutral-700 placeholder-neutral-400 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500 focus-visible:border-blue-500"
                  @keydown.enter.prevent="handleCreateCategory(torrent)"
                />
                <ContextMenuSeparator class="dark:bg-neutral-700" />
                <ContextMenuCheckboxItem
                  class="hover:bg-blue-500/80"
                  :model-value="!torrent.category"
                  @click="store.setCategory({ hashes: [torrent.hash], category: '' })"
                >
                  None
                </ContextMenuCheckboxItem>
                <ContextMenuCheckboxItem
                  v-for="(cat, name) in store.categories"
                  :key="name"
                  class="hover:bg-blue-500/80"
                  :model-value="torrent.category === name"
                  @click="store.setCategory({ hashes: [torrent.hash], category: name })"
                >
                  {{ name }}
                </ContextMenuCheckboxItem>
              </ContextMenuSubContent>
...
const filterLabel = computed(() => TorrentFilterLabels[store.filter] || store.filter)
const newCategory = ref('')

async function handleCreateCategory(torrent) {
  const name = newCategory.value.trim()
  if (!name) return
  await store.addCategory({ name, savePath: '' })
  await store.setCategory({ hashes: [torrent.hash], category: name })
  newCategory.value = ''
}
